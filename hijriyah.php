...
//	echo "DV Deg = " . $DV_deg;	
	...
	//Konjungsi tgl 1 April 2022 jam 13:27:13
	// $secCorrection=1088937 - 86868;
	// $rCor=0;
	...
	// End If
	$tglHijriyahX=$secValue/(24*3600) +1 ; //tgl positif hijriyah 
	$firstSecond=($secValue/(24*3600) - floor($secValue/(24*3600)))*24*3600; //minimal berumur 35,774.71667 sec. atau 2 derajad pd jam 18:00

	// $pndpt=6+3.937421296;  //Konjungsi jam ~14:00 atau jam 18:00 berumur > 2 derajad
    $pndpt=6 + 5.906131944;   //Konjungsi jam ~12:00 atau jam 18:00 berumur > 3 derajad
	$reqDegree=2;
	// echo "first second= $firstSecond \n";
	// echo "sec value = " . $secValue . "\n";
	// // echo "day value = " . $secValue/(24*3600) . "\n";
	// echo "tgl raw hijriyah = " . $tglHijriyahX . "\n";
	if ($firstSecond>$pndpt*3600) { //konjungsi sesudah jam 14:00
		...
		if (floor($tglHijriyahX)==0) {
			...
			$tglHijriyah=floor($tglHijriyahX);
			if ($tglHijriyahX>1) {
				$tglHijriyahX=$tglHijriyahX-1;
				if ($tglHijriyahX< 1 && $tglHijriyahX>0) {
					$tglHijriyahX=30;
					if ($blnHijriyah==1) {
						$thnHijriyah-=1;
						$blnHijriyah=12;
